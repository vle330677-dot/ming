...
export function LondonTowerView({ user, onExit, showToast, fetchGlobalData }: Props) {
  ...
  // 技能抽取状态 (前端模拟每日次数限制，后端建议配合 user.drawCount)
  ...
  const drawSkillBook = () => {
    ...
    // 预留接口，实际需要走后端随机池，这里演示前端逻辑
    setDrawCount(prev => prev + 1);
    const factions = ['物理系', '元素系', '精神系', '感知系', '信息系', '治疗系', '强化系', '炼金系'];
    const randomFaction = factions[Math.floor(Math.random() * factions.length)];
    
    showToast(`【进修成功】获得一本随机中阶技能书：[${randomFaction}·秘要]！`);
    // TODO: 调用后端 /api/users/:id/inventory/add 将书本发给玩家
    fetch(`/api/users/${user.id}/inventory/add`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: `[技能书] ${randomFaction}·秘要`, qty: 1 })
    });
  };
  ...
  return (
    <div className="absolute inset-0 bg-slate-100 overflow-hidden font-sans select-none text-slate-800">
      ...
      <div 
        ...
        style={{ backgroundImage: "url('/伦敦塔.jpg')" }}
      >
        <div className="absolute inset-0 bg-gradient-to-b from-sky-50/50 via-white/20 to-sky-100/50 pointer-events-none mix-blend-overlay"></div>
      </div>
