                    ...
                    {isArmy && (
                      <div className="space-y-4 pt-6 border-t border-white/10">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <button onClick={fetchDisputes} className="p-2 text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors"><RefreshCcw size={16}/></button>
                        </div>
                        {disputes.length === 0 ? (
                          <div className="text-center p-8 bg-slate-800/30 rounded-xl border border-dashed border-slate-700">
                            <p className="text-sm text-slate-500">当前军营内外太平，无纠纷上报。</p>
                          </div>
                        ) : (
                          disputes.map(d => (
                            <div key={d.id} className="p-4 bg-slate-800/80 border border-amber-500/20 rounded-xl hover:border-amber-500/50 transition-colors">
                              <div className="flex justify-between items-start mb-2">
                                ...
                                <span className="text-[10px] px-2 py-0.5 bg-amber-500/10 text-amber-500 rounded border border-amber-500/20">纠纷</span>
                              </div>
                              <p className="text-xs text-slate-400 mb-4 leading-relaxed line-clamp-2">{d.content}</p>
                              
                              {d.acceptedById === user.id ? (
                                <div className="flex gap-2">
                                  <button onClick={() => resolveDispute(d.id)} className="flex-1 py-2 bg-emerald-600 text-white text-xs font-black rounded-lg hover:bg-emerald-500 shadow-lg shadow-emerald-500/20">纠纷解决 (+1000G)</button>
                                  <button onClick={() => releaseDispute(d.id)} className="flex-1 py-2 bg-slate-700 text-slate-300 text-xs font-black rounded-lg hover:bg-slate-600">放弃</button>
                                </div>
                              ) : d.status === 'accepted' ? (
                                <p className="text-xs text-amber-500 font-bold text-center bg-amber-500/10 py-2 rounded-lg border border-amber-500/20">已被军官 {d.acceptedByName} 接手</p>
                              ) : (
                                <button onClick={() => acceptDispute(d.id)} className="w-full py-2.5 bg-amber-600 text-white text-xs font-black rounded-lg hover:bg-amber-500 shadow-lg shadow-amber-600/20 uppercase tracking-wide">介入调解</button>
                              )}
                            </div>
                          ))
                        )}
