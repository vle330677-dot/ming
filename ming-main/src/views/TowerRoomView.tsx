...
import {
  X, Heart, Zap, DoorOpen, Camera,
  Lock, Download, FileText, Settings, Home, EyeOff, MapPin
} from 'lucide-react';
...
export function TowerRoomView({ currentUser, homeOwner, spiritStatus, onClose, showToast, onUpdateData }: Props) {
  ...
  const exportLogs = (archive?: any) => {
    ...
    targets.forEach((arc) => {
      ...
      (arc.messages || []).forEach((m: any) => {
        if (m.type === 'system') text += `[系统广播]: ${m.content}\n\n`;
  ...
  const saveSettings = async (updatedData = roomData) => {
    const res = await fetch(`/api/users/${currentUser.id}/settings`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        roomBgImage: updatedData.bgImage,
        roomDescription: updatedData.description,
        allowVisit: updatedData.allowVisit,
    ...
    return (
      <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900 border border-slate-700 p-10 rounded-3xl text-center max-w-sm">
          ...
          <button onClick={onClose} className="px-6 py-2 bg-slate-700 text-white font-bold rounded-full hover:bg-slate-600">
            离开
          </button>
        </motion.div>
      </div>
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white rounded-[32px] w-full max-w-3xl h-[80vh] shadow-2xl flex overflow-hidden relative">
        ...
        <div className="flex-1 relative bg-slate-50 overflow-y-auto">
          <button onClick={onClose} className="absolute top-6 right-6 z-50 p-2 bg-black/40 backdrop-blur-md text-white rounded-full hover:bg-black/60">
            <X size={18} />
          </button>
          ...
          {activeTab === 'spirit' && isOwner && spiritStatus && (
            <div className="p-10 min-h-full flex flex-col items-center justify-center">
              ...
              <div className="flex gap-4">
                <button onClick={() => handleAction('interact-spirit', { intimacyGain: 5 })} className="px-6 py-3 bg-pink-50 text-pink-600 font-black rounded-xl border border-pink-100">
                  摸摸 (+5)
                </button>
                <button onClick={() => handleAction('interact-spirit', { intimacyGain: 10 })} className="px-6 py-3 bg-amber-50 text-amber-600 font-black rounded-xl border border-amber-100">
                  喂食 (+10)
                </button>
                <button onClick={() => handleAction('interact-spirit', { intimacyGain: 15 })} className="px-6 py-3 bg-indigo-50 text-indigo-600 font-black rounded-xl border border-indigo-100">
                  训练 (+15)
                </button>
              </div>
            </div>
            ...
            <div className="p-10 h-full overflow-y-auto">
              ...
              <div className="space-y-8 max-w-lg">
                <div className="p-5 bg-sky-50 border border-sky-100 rounded-2xl">
                  ...
                  <p className="text-[10px] text-sky-700 mb-4">设置密码后，登录同名角色需要密码验证。</p>

                  {isEditingPassword ? (
                    <div className="space-y-3">
                      <input
                        type="text"
                        placeholder="输入新密码（留空则取消密码）"
                        value={tempPassword}
                      ...
                      <div className="flex gap-2">
                        <button onClick={confirmChangePassword} className="flex-1 py-2 bg-sky-600 text-white font-bold rounded-xl hover:bg-sky-700 text-sm transition-colors">
                          确认修改
                        </button>
                        <button onClick={() => setIsEditingPassword(false)} className="flex-1 py-2 bg-white text-sky-600 font-bold rounded-xl border border-sky-200 hover:bg-sky-50 text-sm transition-colors">
                          取消
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-sky-100">
                      <span className="text-sm font-bold text-slate-500 tracking-widest">{roomData.password ? '••••••••' : '未设置密码'}</span>
                      <button onClick={() => { setIsEditingPassword(true); setTempPassword(roomData.password); }} className="px-4 py-1.5 bg-slate-100 text-slate-600 text-xs font-black rounded-lg hover:bg-slate-200 transition-colors">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500">关闭后其他玩家无法查看你的房间</p>
                    </div>
                    <button
