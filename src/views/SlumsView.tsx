...
export function SlumsView({ user, onExit, showToast, fetchGlobalData }: Props) {
  ...
  // --- 繁荣度计算 ---
  const westResidents = allPlayers.filter(p => p.currentLocation === 'slums').length; // 简化为当前所在地人数模拟
  ...
  const checkQualifications = (targetRank: string) => {
    ...
    if (targetRank === ROLES.CITIZEN) return true; // 技工无门槛
  ...
  const handleMoveIn = async () => {
    ...
    // 这里简化逻辑，假设 location 即为家
    showToast('已在西区登记常住人口！城市繁荣度 +100。'); 
    // 实际项目中应调用后端保存 homeLocation
  };
                  ...
                  <div className="space-y-6">
                    {!isWestSide ? (
                      ...
                      <div className="text-center p-6 bg-stone-900 border border-stone-700 rounded-xl">
                        ...
                        <button 
                          onClick={() => { if(confirm("确定要辞去西区职务吗？")) fetch('/api/tower/quit', { method:'POST', body:JSON.stringify({userId:user.id}), headers:{'Content-Type':'application/json'}}).then(() => {showToast("已辞职。"); fetchGlobalData(); setSelectedBuilding(null);}) }}
