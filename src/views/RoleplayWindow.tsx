...
import { X, Send, Minus, CheckCircle, Shield, AlertTriangle, Save, MessageCircle } from 'lucide-react';
...
export function RoleplayWindow({ sessionId, currentUser, onClose }: Props) {
  ...
  return (
    // 【修复1】将统一的拖拽边界容器放在最外层，并加上 overflow-hidden 确保不越界
    <div ref={dragConstraintsRef} className="fixed inset-0 pointer-events-none z-[9000] overflow-hidden">
      {isMinimized ? (
        <motion.div
          drag
          dragConstraints={dragConstraintsRef}
          dragMomentum={false}
          dragElastic={0} // 【修复2】取消弹性拖拽，严禁超出屏幕边缘
          // 【修复3】改为 absolute 定位，使其受限于父级边界容器
          className="absolute top-1/4 right-8 bg-sky-900 border border-sky-500 rounded-full p-3 shadow-[0_0_20px_rgba(14,165,233,0.4)] cursor-pointer hover:bg-sky-800 transition-colors pointer-events-auto"
