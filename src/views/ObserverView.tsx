      ...
      <AnimatePresence>
        {selectedBuilding && (
          <>
            ...
            <motion.div 
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={() => {setSelectedBuilding(null); setIsWriting(false);}}
            ...
            >
              <div className="bg-black w-full max-w-3xl shadow-[0_0_50px_rgba(34,197,94,0.15)] relative border border-green-800 p-1 flex flex-col max-h-[85vh] pointer-events-auto">
                ...
                <button onClick={() => {setSelectedBuilding(null); setIsWriting(false);}} className="absolute top-6 right-6 text-green-700 hover:text-green-400 transition-colors z-20 bg-black/50 p-1">
                  <X size={24}/>
                </button>
                      ...
                      {libraryTab === 'books' && (
                        <div className="space-y-4">
                          ...
                          {isWriting && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-4 border border-green-500 bg-green-900/20 space-y-3 mb-6">
                              ...
                              <div className="flex gap-2">
                                <button onClick={handlePublishBook} className="flex-1 py-2 bg-green-700 text-black font-bold hover:bg-green-600 uppercase">UPLOAD</button>
                                <button onClick={() => setIsWriting(false)} className="flex-1 py-2 bg-transparent text-green-600 font-bold border border-green-800 hover:border-green-600 uppercase">CANCEL</button>
                              </div>
                            </motion.div>
                          )}
                          ...
                          <div className="grid grid-cols-1 gap-4 max-h-72 overflow-y-auto pr-2 custom-scrollbar">
                            {books.map((book, idx) => (
                              <div key={idx} className="p-5 border border-green-900/50 bg-black/50 hover:border-green-500 transition-colors group cursor-default">
                                <div className="flex justify-between items-start mb-3">
                                  <h3 className="font-bold text-green-400 text-lg group-hover:text-white transition-colors">{book.title}</h3>
                                  <span className="text-[10px] text-green-700 bg-green-900/10 px-2 py-1 border border-green-900/30">AUTH: {book.author}</span>
                                </div>
