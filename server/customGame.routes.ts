...
async function createAnnouncement(
  ...
  await dbRun(
    ...
    [type, title, content, toJson(payload, {})]
  );
}
  ...
  router.post("/:id/vote/open", requireAdmin, async (req, res) => {
    try {
      ...
      await createAnnouncement(
        ...
        "vote_open",
        `【开票】自定义活动 #${gameId}`,
        `《${game.title}》开局投票已开启，${durationMinutes}分钟后关票。`,
    ...
    try {
      ...
      await dbRun(
        ...
        `INSERT INTO custom_game_run_events(run_id, actor_user_id, event_type, message, payload, created_at)
         VALUES (?, ?, 'run_start', ?, ?, ?)`,
        [runId, admin.id, "副本已开始", toJson({ gameId, yes, no }, {}), ts]
      );
